...
import type { avatar } from '~/types/RichTypes'
...
const boxStyle = computed(() => {
  ...
  const borderColor = props.boxData.color?.replace('bg-', 'border-').replace(/-\d{3}/, '-600')
...
<template>
  <div
    class="h-32 w-32 touch-none select-none border-4 rounded-lg"
